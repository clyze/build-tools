...
class RepackagePlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        if (project.plugins.hasPlugin('java')) {
            ...
            throw new RuntimeException(msg("One of these plugins should be applied before the ${NAME} plugin: java, android, com.android.application, com.android.library"))
        }

        // Create the plugin extension that will hold all configuration.
        project.extensions.create('repackage', Extension)
    ...
    private synchronized void configureSourceJarTask(Project project) {
        ...
        if (existing == null) {
            ...
            throw new RuntimeException(msg("Non-JAR task ${TASK_SOURCES_JAR} exists (of group ${existing.group}), cannot configure ${NAME} plugin."))
        }
